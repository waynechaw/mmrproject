...
$(".reset-tracking" ).on( "click" , function() {
...
function getMasteryData(name, tag, selectedMappedRegion) {
  ...
  $.ajax({
    ...
    success: function(resp) {
      ...
      $('.last-champ-index').text(resp.data.length + "th");

      $('.last-champ').text(champData[masteryData[resp.data.length - 1].championId].name);
      ...
      $('.start-date-value').text( moment(profiles[objKey].dateStarted).format('MMMM Do YYYY') );
      $('.elapsed-value').text( moment(profiles[objKey].dateStarted).fromNow(true) );
